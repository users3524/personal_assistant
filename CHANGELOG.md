...
> 说明：本文件保留历史版本记录。当前功能、路由、数据库表和模块边界以 `README.md` 与 `docs/` 下的规格文档为准；当前简历导出为 PNG 分享而非 PDF，数据库为 `schemaVersion = 15`、20 张表，文玩估值已应用层下线并保留兼容壳。
...
- 新增复盘月度日历入口，支持按日期查看日报状态与历史沉淀。
- 落地 AI 复盘后续底座：生成任务冷表、高光表、多源关系、项目高光关系和本地向量表。
- 简历模块补充高光/STAR/PDF 后续工程策略，为证据化输出闭环做准备。

### 🐛 修复

- 修复向量表 `vector_embeddings` 建表约束在真机 SQLite 上把字符串误识别为列名的问题。

### 📝 文档

- 基于 2026-06-21 代码现状重整全部 docs，全局区分“已实现事实”和“后续规划”。
- 重新收敛产品主线为“行动/兴趣素材 -> 每日复盘 -> 高光证据池 -> 简历/长期记忆”。
- 重置 TODO 为下一阶段 backlog，优先推进复盘生成闭环、备份安全、高光证据池和简历输出。
