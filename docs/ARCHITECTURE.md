...
| 文玩/盘串 | 藏品档案、分类字段、照片、盘玩打卡、打卡对比、月历、趣味榜单；估值功能已应用层下线，schema v7 遗留表/列暂留兼容。 |
...
`test/app_test.dart` 和 `test/widget_test.dart` 当前都是占位测试，只断言 `true`。尚未覆盖 DAO、Provider、页面或迁移逻辑。
