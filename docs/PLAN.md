...
| 待办 | 已有任务树、子任务、软删除、归档、统计；树查询已批量化，重新打开可清空完成/取消时间；今日统计、本周完成率、拖延率已有专项单元测试。 |
...
1. 为待办批量树查询评估是否需要补充 `parent_id`、`list_id`、`deleted_at/status/due_date` 等索引。
2. 评估文玩榜单从 Provider 循环计算迁移到 DAO 聚合查询，并按查询计划补索引。
