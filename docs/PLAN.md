...
| AI 复盘 | 已有独立 `/review` 历史入口、对话式日报和 ISO 周报；日报会读取当日文玩盘玩分钟，文本输入限 500 字，语音输入 60 秒自动截断；PromptBuilder 已负责字符/token 预算；`chat_turns` 已落地并支持每日 15 轮云端请求限制；深夜 raw context pack 字段契约与 8000 字优先级裁剪器已落地，未实现深夜后台。 |
