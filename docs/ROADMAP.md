...
| 文玩 | 藏品、照片、盘玩打卡、月历、对比、趣味榜单；估值功能已应用层下线，遗留表/列暂留兼容。 |
...
| 工作 | 说明 |
| --- | --- |
| 图片路径统一 | UI、分享、保存、备份导出统一通过单一图片解析入口处理相对/绝对路径。 |
| 长串分类 | 将“长串”作为默认文玩分类补入分类初始化、分类管理和文档。 |
| 估值物理移除评估 | 当前已应用层下线；后续旧备份归档路径稳定后，再评估是否迁移删除 `valuation_records` 和 `current_valuation`。 |
| 榜单性能优化 | 当前趣味榜单多在 Dart 内存循环计算；后续按数据规模评估 DAO 聚合和 `patting_logs(item_id, date DESC)` 索引。 |
