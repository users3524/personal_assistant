...
| 模块 | 基线 |
| --- | --- |
| 待办 | 周/月视图、任务树、子任务、软删除、归档、统计。 |
| 文玩 | 藏品、照片、盘玩打卡、月历、对比、趣味榜单；估值代码仍存在。 |
| AI 复盘 | 对话式日报、周报、离线/在线 AI。 |
| 简历 | 三模板预览、编辑、拖拽排序、可见性、图片导出。 |
| 设置 | AI 配置、通知、分类管理、JSON 备份。 |
...
| 文玩图片路径统一 | 统一绝对/相对路径策略，UI 不再混用 `File(path)` 与 `resolveImageFile()`。 |
| 估值模块移除计划 | 估值不再作为保留模块，需移除展示、Provider、DAO、表或做迁移。 |
| 复盘页限制补齐 | 若继续执行原设计，需要新增 turn 计数、文本长度限制、语音时长限制。 |
| 测试补齐 | 当前测试是占位，需要 DAO、Provider、迁移、备份恢复测试。 |
| API Key 安全存储 | 当前明文在 `user_preferences.ai_api_key`；后续应接入平台安全存储，并保持 Drift 中只保存非敏感策略配置。 |

## 4. 下一版 Schema 草案

以下是智能化迁移的 schema 草案，不属于当前 `schemaVersion = 6`。
...
| 工作 | 说明 |
| --- | --- |
| 树查询批量化 | 当前 `getTree()` 是父任务逐个查子任务；改为一次查父任务、一次 `IN` 查直接子任务，再在 Dart 内存分组注入。 |
| 待办索引 | 后续结合查询计划补 `parent_id`、`list_id`、活跃任务查询相关索引。 |
| 清单 UI 完整化 | `todo_lists` 已有表和 DAO，UI 仍使用有限。 |
| 重新打开状态修复 | 当前 `copyWith` 无法显式置空完成/取消时间；可用 `Value<T?>`/哨兵对象解决 nullable 清空歧义。 |
| 级联事务 | 父子任务状态和软删除建议用事务包裹。 |

## 6. P1：文玩模块瘦身

| 工作 | 说明 |
| --- | --- |
| 移除估值图表 | 删除 `ValuationChart` 和 `fl_chart` 依赖前，确认没有其它图表使用。 |
| 移除估值榜单 | 财富榜、潜力榜等估值相关内容替换为盘玩/照片/时间维度。 |
| 数据迁移 | 估值模块不保留；迁移前需决定是否把 `valuation_records.amount/remark/date` 归档追加到 `antique_items.description` 或 `notes`。 |
| 备份同步 | 估值移除后同步更新 `BackupService` 表清单。 |
