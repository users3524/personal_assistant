...
| `ReviewGenerationJobExecutor` | 前台执行 pending 任务入口：生成并保存 `raw_assets_dump`，App 启动补偿和复盘页手动触发共用，按 `targetDate` 合并并发。 |
...
当前取舍：深夜引擎已实现前台 Catch-Up Guard、Android WorkManager 注册、任务冷表、结构化 runner、真实素材组包、raw pack 裁剪和前台 pending job 执行入口；WorkManager 回调当前只做安全空执行，AI 生成和日报写入仍按 TODO 分步接入。桌面/Web 不追求后台常驻。
