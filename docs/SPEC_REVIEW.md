...
注意：结构化 JSON -> 修复 JSON -> 纯文本降级的 runner 已存在，但尚未接入真实 raw pack、AI 服务和日报写入闭环。
...
当前取舍：深夜引擎已实现前台 Catch-Up Guard、Android WorkManager 注册、任务冷表和结构化 runner；WorkManager 回调当前只做安全空执行，真实素材组包、AI 生成和日报写入仍按 TODO 分步接入。桌面/Web 不追求后台常驻。
...
| 工作 | 原因 | 验收 |
| --- | --- | --- |
| RawContextPackBuilder | 当前只有字段契约和裁剪器，没有真实组包 | 目标日期下待办、chat_turns、日报草稿、patting_logs 可组成 JSON |
