...
| `NightlyStructuredReviewRunner` | 已实现初次 JSON、一次 JSON 修复、一次纯文本降级的 3 次调用上限；失败后标记任务 failed 并设置日报校准状态。 |
...
注意：结构化 JSON -> 修复 JSON -> 纯文本降级的 runner 已存在，`RawContextPackBuilder` 已能生成真实 raw pack；尚未接入 AI 服务和日报写入闭环。
...
当前取舍：深夜引擎已实现前台 Catch-Up Guard、Android WorkManager 注册、任务冷表、结构化 runner、真实素材组包和 raw pack 裁剪；WorkManager 回调当前只做安全空执行，AI 生成和日报写入仍按 TODO 分步接入。桌面/Web 不追求后台常驻。
...
`RawContextClipper` 已实现首版裁剪策略，默认预算为 8000 字符。排序规则优先保留高优已完成任务，其次保留教练式对话/云端 user turn、有备注的文玩打卡、日报草稿和普通离线便签；同分素材按 `created_at` 倒序保留较新的内容。空内容会被丢弃，超预算素材整体丢弃而不拆分。`RawContextPackClipper` 已接入真实 raw pack，输出仍是可解析 JSON，并在 `clip` 中记录 `input_chars`、`kept_chars`、保留项摘要和丢弃原因；尚未接入 pending job 执行入口。
...
| 工作 | 原因 | 验收 |
| --- | --- | --- |
| Pending Job 执行入口 | `review_generation_jobs` 需要从状态表变成任务表 | App 启动补偿和复盘页手动触发共用同一入口 |
| Runner 接入 AIService | 结构化 runner 已有，但没有连到实际模型调用 | 成功/失败/降级路径都有测试 |
| 写入日报与校准 | 生成结果必须落到用户可见页面 | success 写入日报，failed 标记 `calibration_required` |
