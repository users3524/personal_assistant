...
输出形态首版为 JSON 对象，至少包含 `target_date`、`generated_at`、`timezone_offset_minutes` 和上述分区数组。隐私边界：API Key、图片 base64、备份文件路径、平台安全存储引用都不得进入 raw pack。素材预算裁剪不在本设计项内实现，下一步按约 8000 字预算做优先级裁剪。
