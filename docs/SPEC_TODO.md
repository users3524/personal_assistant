...
| 清单 | `todo_lists` 表、DAO、列表筛选、管理弹窗、表单选择和详情展示已接入。 | 后续可继续做清单排序、批量移动和更细分视图。 |
...
级联状态和软删除已使用 Drift `transaction` 包裹，保证父任务、直接子任务一致提交。未来接入多态高光关联后，事务内还需要同步清理 `milestone_relations`。
